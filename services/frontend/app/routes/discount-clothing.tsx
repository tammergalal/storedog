...
export async function loader() {
  try {
    ...
    return json({ clothingProducts: clothingProducts.slice(0, 12) })
  } catch (error) {
    console.error('Error fetching clothing products:', error)
    return json({ clothingProducts: [] })
  }
}

export default function DiscountClothingPage() {
  const { clothingProducts } = useLoaderData<typeof loader>()
      ...
      <Container className="pt-8 pb-16">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Welcome to our fabulous discount clothing collection! You clicked on
            the Discount Clothing ad and landed here. Discover stylish fashion
        ...
        {clothingProducts.length > 0 ? (
          <>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {clothingProducts.map((product: any) => (
                <ProductCard
                  key={product.id}
                  product={product}
                  variant="simple"
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              We're still building our clothing collection! Check back soon for
              fabulous fashion items.
            </p>
          </div>
        )}
